...
import { Shield, Mail, Lock, Eye, EyeOff, ArrowRight, Building, UserPlus } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(messages[err.code] || err.message || 'Sign-in failed');
    }
    setLoading(false);
  };
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err.message === 'NO_PROFILE') {
        toast.error('No account found. Ask your admin for an invite, or register a new company.');
      } else {
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-brand-600 via-brand-700 to-brand-900 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-center px-16">
          ...
          <p className="text-lg text-brand-200 max-w-md leading-relaxed">
            Schedule shifts, track costs, manage projects and payments — all in one beautifully designed platform.
          </p>
          <div className="mt-12 grid grid-cols-3 gap-6">
            {[
              { value: 'Free', label: 'Up to 4 workers' },
              { value: '€2/mo', label: 'Per worker after 5' },
              { value: '€99/mo', label: 'Enterprise 21+' },
            ].map((item) => (
              <div key={item.value} className="bg-white/10 backdrop-blur-sm rounded-xl p-4">
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-12 bg-surface-50">
        <div className="w-full max-w-md">
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white border border-surface-200 rounded-xl
                       text-sm font-medium text-surface-700 hover:bg-surface-50 hover:border-surface-300 transition-all shadow-card"
          >
            ...
            <div className="p-4 bg-white border border-surface-200 rounded-xl">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <Link href="/register" className="text-xs font-semibold text-brand-600 hover:text-brand-700 mt-1.5 inline-block">
            ...
            <div className="p-4 bg-white border border-surface-200 rounded-xl">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <Link href="/join" className="text-xs font-semibold text-emerald-600 hover:text-emerald-700 mt-1.5 inline-block">
