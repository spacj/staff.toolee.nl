  ...
  return (
    <Layout>
      <div className="space-y-6">
        ...
        {/* Clock In/Out */}
        <div className={cn('card p-6 text-center', isCurrentlyIn ? 'bg-gradient-to-br from-emerald-50 to-emerald-100/50 border-emerald-200' : '')}>
          <div className={cn('w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4', isCurrentlyIn ? 'bg-emerald-100' : 'bg-surface-100')}>
            <Clock className={cn('w-10 h-10', isCurrentlyIn ? 'text-emerald-600' : 'text-surface-400')} />
          </div>
          {isCurrentlyIn ? (
            <>
              <p className="text-lg font-display font-bold text-emerald-800">You're clocked in</p>
              <p className="text-sm text-emerald-600 mt-1">Since {active.entries?.filter(e => !e.clockOut)?.[0]?.clockIn?.slice(11, 16) || '—'}</p>
              ...
              <button onClick={handleClockOut} className="btn-danger mt-5 !py-3 !px-8"><Square className="w-4 h-4" /> Clock Out</button>
            </>
          ) : (
            <>
              ...
              <button onClick={handleClockIn} className="btn-primary mt-5 !py-3 !px-8"><Play className="w-4 h-4" /> Clock In</button>
            </>
          )}
        </div>
