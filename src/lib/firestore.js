...
export async function syncOrgPlan(orgId) {
  ...
  await upd(C.ORGANIZATIONS, orgId, { plan: newTier, activeWorkerCount: workers.length, shopCount: shops.length });

  // Trigger PayPal subscription quantity sync (fire-and-forget, won't break if API isn't set up)
  try {
  ...
  } catch (e) { /* API route may not exist in dev — ignore */ }
