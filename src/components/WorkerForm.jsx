...
export default function WorkerForm({ worker, onSuccess, onCancel }) {
  ...
  const [form, setForm] = useState({
    ...
    position: worker?.position || '', role: worker?.role || 'worker', status: worker?.status || 'active',
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div><label className="label">Role</label><select name="role" value={form.role} onChange={handleChange} className="select-field" disabled={!isAdmin}><option value="worker">Worker</option><option value="manager">Manager</option>{isAdmin && <option value="admin">Admin</option>}</select></div>
